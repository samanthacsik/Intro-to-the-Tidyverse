...
site_phenometrics_data$newdate <- as.Date(site_phenometrics_data$Mean_First_Yes_DOY, origin = site_phenometrics_data$origin, tz = "UTC")

# the stack overflow tip page: https://stackoverflow.com/questions/46313666/how-to-convert-day-of-year-and-year-to-a-standard-date-format

#### add year and month columns ####
final_df <- site_phenometrics_data %>% 
  mutate(Year = year(newdate),
         Month_no = month(newdate),
         Month = case_when(
           Month_no == "1" ~ "January",
           Month_no == "2" ~ "February",
           Month_no == "3" ~ "March",
           ...
           Month_no == "7" ~ "July",
           Month_no == "8" ~ "August",
           Month_no == "9" ~ "September",
           Month_no == "10" ~ "October", 
           Month_no == "11" ~ "November",
           Month_no == "12" ~ "December"
         )) %>% 
...
```{r multiple plant species}
desert_flowers <- final_df %>% 
  unite("Genus_Species", Genus, Species) %>% 
  filter(Genus_Species == "Yucca_brevifolia" |
           Genus_Species == "Larrea_tridentata" |
           Genus_Species == "Yucca_schidigera")
```
