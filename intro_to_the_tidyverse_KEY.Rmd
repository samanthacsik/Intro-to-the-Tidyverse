...
The last thing we need to do is set the levels for variables in the `pheno` column. We can do this with `mutate()`.

```{r}
relevel <- jt_tally %>% 
  mutate(Pheno = factor(Pheno, c("Flowers or flower buds", "Open flowers", "Pollen release (flowers)")))
```
