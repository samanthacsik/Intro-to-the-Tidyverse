...
Let's pretend we're trying to plan a getaway to the California desert and want to time our trip so that we have the greatest chance of seeing fully bloomed plants. 
...
This dataset is huge, but we want to wrangle it so that it only includes the information that we're interested in. In order, we'll
...
This is a bit more manageable (`r nrow(phenometrics_data)` rows as opposed to `r nrow(ca_pheno)` rows) but there are still a lot of columns that we don't necessarily need. Let's select the columns we're interested in.
...
We split each wrangling step up into a separate data frame, but you could have done all these commands together in one chunk. 
...
We love **joshua trees** (*Yucca brevifolia*) and **creosote bushes** (*Larrea tridentata*) and are most interested in when these plants are blooming. Let's first isolate data for each of these species by

a. filtering for only the species of interest  
b. finding the total counts for these species for each month throughout the year

```{r, message = FALSE}

# data for just joshua trees
joshua_tree <- ca_pheno %>% 
  filter(Genus_Species == "Yucca_brevifolia") %>% 
  group_by(Month, Pheno) %>% 
  tally()

# column plot of joshua trees
joshua_tree_plot <- ggplot(joshua_tree, aes(x = Month, y = n, fill = Pheno, fct_relevel("Flowers or flower buds", "Open flowers", "Pollen release (flowers)"))) +
  geom_col(position = "dodge") # "dodge" creates separate columns for each pheno, rather than a stacked column

joshua_tree_plot

# data for just creosote bush
creosote_bush <- ca_pheno %>% 
  filter(Common_Name == "creosote bush") %>% 
  group_by(month, pheno) %>% 
  summarize( 
    total_counts = length(pheno) # same as tally!
  )

# column plot of creosote bushes
creosote_plot <- ggplot(creosote_bush, aes(x = month, y = total_counts, fill = pheno, fct_relevel("Flowers or flower buds", "Open flowers", "Pollen release (flowers)"))) +
  geom_col(position = "dodge") 
```

```{r}
# add column to 'joshua_tree' with name; rename 'n' to 'total_counts' so that it matches 'creosote_bush'
joshua_tree_name <- joshua_tree %>% 
  mutate(species = "joshua tree") %>% 
  rename(total_counts = n)

# add column to 'creosote_bush' with name
creosote_bush_name <- creosote %>% 
  mutate(species = "creosote bush") 

# combine 'joshua_tree_name' and 'creosote_bush_name'
jt_cb <- bind_rows(joshua_tree_name, creosote_bush_name) %>% 
  group_by(species, pheno)

# plot
ggplot(jt_cb, aes(x = month, y = total_counts, color = species, shape = pheno)) +
  geom_point(size = 3) +
  geom_line()
```
