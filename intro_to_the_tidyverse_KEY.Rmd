...
## **Don't have the tidyverse/janitor yet?**
...
pizza_data <- tribble(
  ~name,    ~"year 2017",   ~"year 2018",   ~"year 2019",
  "Sam",       25,        20,        16,
  "An",        20,        15,        11,
  "Allison",   18,        17,        10,
  "Julie",     19,        10,        14,
...
We'll first want to make the column names more user-friendly by replacing spaces with underscores. We do this with `janitor::clean_names()`. 
```{r}
clean_columns <- pizza_data %>% 
  clean_names(case = c("snake"))
```

Then, we'll want to transform 'pizza_data' into tidy (long) format:

**gather():** transforms data from wide to long format
```{r}
tidy_pizza <- clean_columns %>% 
  gather(year_2017, year_2018, year_2019, key = "year", value = "pizzas_eaten")
```
