...
In order to wrangle our data, we will first
...
(d) merge genus and species into a single column
(e) remove missing values with NA
...
ca_pheno <- pheno_data %>%
  ...
  filter(State == "CA")  %>% 
  rename(pheno = Phenophase_Description) %>% 
  unite("Genus_Species", Genus, Species, sep = "_", remove = TRUE)  %>% # merge genus and species into same column
  na_if(-9999) %>% 
  drop_na() %>% 
...
```{r}
joshua_tree_np_flowers <- ca_pheno %>% 
  filter(Common_Name == "Joshua tree" | Common_Name == "creosote bush" | Common_Name == "Mojave yucca") %>% 
  group_by(month_name, Common_Name, pheno) %>% 
  tally()

ggplot(joshua_tree_np_flowers, aes(x = month_name, y = n, fill = pheno)) +
  geom_col(position = "dodge") +
  fct_relevel("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", ) %>% 
