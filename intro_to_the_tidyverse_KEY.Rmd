...
ca_pheno <- pheno_data %>% 
  ...
  drop_na(Year, Month) %>% 
  mutate(
    Pheno = fct_relevel(Pheno, c("Flowers or flower buds", "Open flowers", "Pollen release (flowers)")),
    Month = fct_relevel(Month, c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December")))
```

### Plot:

With this data set, we're ready to subset for desert species we're interested in. We love **joshua trees** (*Yucca brevifolia*) and **creosote bushes** (*Larrea tridentata*) and are most interested in when these plants are blooming. Let's first isolate data for each of these species by
...
creosote_plot <- ggplot(creosote_bush, aes(x = Month, y = total_counts, fill = Pheno)) +
